  ...
  <div class="return-page">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button type="primary" :icon="Plus" @click="handleAdd">新建退货单</el-button>
          </div>
        </div>
      </template>
      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane label="采购退货" name="1" />
        <el-tab-pane label="销售退货" name="2" />
      </el-tabs>
      ...
      <el-table :data="tableData" stripe border v-loading="loading">
        ...
        <el-table-column prop="return_type" label="类型" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.return_type === 1 ? 'warning' : 'primary'" size="small">
              {{ row.return_type === 1 ? '采购退货' : '销售退货' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="related_no" label="关联单号" width="150" />
        <el-table-column prop="partner_name" label="供应商/客户" min-width="150">
          <template #default="{ row }">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="退货类型" prop="return_type">
              <el-select v-model="formData.return_type" placeholder="选择退货类型" style="width: 100%" @change="onReturnTypeChange">
          ...
          <el-col :span="8">
            <el-form-item v-if="formData.return_type === 1" label="供应商" prop="supplier_id">
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="退货单详情" width="850px" destroy-on-close>
      <el-descriptions :column="3" border>
        <el-descriptions-item label="退货单号">{{ currentOrder.return_no }}</el-descriptions-item>
        <el-descriptions-item label="退货类型">
          <el-tag :type="currentOrder.return_type === 1 ? 'warning' : 'primary'" size="small">
            {{ currentOrder.return_type === 1 ? '采购退货' : '销售退货' }}
          </el-tag>
        ...
        <el-descriptions-item label="供应商/客户">
          {{ currentOrder.supplier_name || currentOrder.customer_name || '-' }}
        </el-descriptions-item>
        <el-descriptions-item label="退款金额">
          <span style="color: #f56c6c; font-weight: bold">¥{{ currentOrder.refund_amount?.toFixed(2) }}</span>
        </el-descriptions-item>
        <el-descriptions-item label="退货原因" :span="2">{{ currentOrder.return_reason || '-' }}</el-descriptions-item>
...
const rules = {
  return_type: [{ required: true, message: '请选择退货类型', trigger: 'change' }],
...
const fetchSuppliers = async () => {
  try {
    const res = await request.get('/suppliers', { params: { page_size: 1000 } })
    if (res.code === 200) {
...
const fetchProducts = async () => {
  try {
    const res = await request.get('/products', { params: { page_size: 1000 } })
    if (res.code === 200) {
...
const handleAdd = () => {
  ...
  Object.assign(formData, {
    return_type: Number(activeTab.value),
    related_no: '',
    supplier_id: null,
    customer_id: null,
    return_reason: '',
...
const onProductChange = (productId, index) => {
  ...
  if (product) {
    ...
    formData.items[index].unit_price = product.purchase_price || product.sale_price || 0
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const data = {
        ...
        supplier_id: formData.return_type === 1 ? formData.supplier_id : null,
        customer_id: formData.return_type === 2 ? formData.customer_id : null,
