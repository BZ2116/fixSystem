<template>
  <div class="return-page">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button type="primary" :icon="Plus" @click="handleAdd">新建采购退货单</el-button>
          </div>
        </div>
      </template>
      ...
      <el-table :data="tableData" stripe border v-loading="loading">
        ...
        <el-table-column label="操作" width="200" fixed="right" align="center">
          <template #default="{ row }">
            ...
            <el-button type="warning" link size="small" @click="handleStockOut(row)" v-if="row.status === 1">出库</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="采购退货单详情" width="850px" destroy-on-close>
...
const handleStockOut = (row) => {
  ElMessageBox.confirm('确定要将该退货单出库吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      const res = await request.post(`/return-orders/${row.id}/stock-out`)
      if (res.code === 200) {
