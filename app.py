...
@app.route('/api/diagnose-encoding', methods=['GET'])
@jwt_required()
def diagnose_encoding():
    ...
    try:
        ...
        for t in types:
            ...
            # 尝试各种编码
            encodings = {}
            for enc in ['utf-8', 'latin-1', 'cp1252', 'gbk', 'gb2312']:
                try:
                    encodings[enc] = name.encode(enc).decode(enc)
                except:
                    encodings[enc] = 'ERROR'
            result.append({
                'id': t.id,
...
@app.route('/api/fix-encoding', methods=['POST'])
@jwt_required()
def fix_encoding():
    """修复数据库中双重UTF-8编码的数据"""
    try:
        ...
        def fix_string(val):
            if not val:
                return val, False
            try:
                # 逐字符还原：cp1252字符用cp1252编码，其他用latin-1编码
                original_bytes = bytearray()
                for ch in val:
                    try:
                        original_bytes.extend(ch.encode('cp1252'))
                    except UnicodeEncodeError:
                        try:
                            original_bytes.extend(ch.encode('latin-1'))
                        except UnicodeEncodeError:
                            return val, False
                fixed = bytes(original_bytes).decode('utf-8')
        ...
        types = AssetType.query.all()
        for t in types:
            fixed_name, is_fixed = fix_string(t.type_name)
            if is_fixed:
                logger.info(f'[FIX] id={t.id}: {repr(t.type_name)} -> {repr(fixed_name)}')
                t.type_name = fixed_name
                fixed_count += 1
        
        # 修复 asset 表
        for a in Asset.query.all():
            for field in ['asset_name', 'customer_name', 'office_name', 'asset_type_name', 'responsible_person', 'remark']:
                val = getattr(a, field, None)
                if val:
                    fixed, is_fixed = fix_string(val)
                    if is_fixed:
                        logger.info(f'[FIX] asset.id={a.id}.{field}: fixed')
                        setattr(a, field, fixed)
                        fixed_count += 1
        
        # 修复 base_customer 表
        try:
            for c in BaseCustomer.query.all():
                for field in ['customer_name', 'contact_name', 'address', 'remark']:
                    val = getattr(c, field, None)
                    if val:
                        fixed, is_fixed = fix_string(val)
                        if is_fixed:
                            logger.info(f'[FIX] customer.id={c.id}.{field}: fixed')
                            setattr(c, field, fixed)
                            fixed_count += 1
        except Exception as e:
            logger.warning(f'修复客户表时出错: {e}')
        
        db.session.commit()
        return jsonify({'code': 200, 'message': f'修复完成，共修复 {fixed_count} 条数据'})
    except Exception as e:
        db.session.rollback()
        logger.error(f'修复编码失败: {str(e)}')
        import traceback
        logger.error(traceback.format_exc())
        return jsonify({'code': 500, 'message': f'修复失败: {str(e)}'}), 500
