...
class InventoryStock(db.Model):
    ...
    product_id = db.Column(BigIntPK, nullable=False)
...
class InventoryLog(db.Model):
    ...
    warehouse_id = db.Column(BigIntPK)  # 仓库ID
    ...
    order_id = db.Column(BigIntPK)  # 关联单据ID
    ...
    operator_id = db.Column(BigIntPK)  # 操作人ID
...
class CostAdjust(db.Model):
    ...
    remark = db.Column(db.Text)  # 备注
    created_by = db.Column(BigIntPK)  # 创建人ID
    created_by_name = db.Column(db.String(50))  # 创建人
    audited_by = db.Column(BigIntPK)  # 审核人ID
